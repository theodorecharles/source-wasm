...
- A canonical wasm-game-framework 0.7.1 suite/locked deployment contract.
- Exact owner-data validation into `/data` and browser IndexedDB caching.
...
there is no legitimate complete engine currently available to compile into an
HL2 browser client. Counter-Strike: Source also lacks an official published
game-code basis in Source SDK 2013.

## Safe continuation

1. Keep `source-lock.json` immutable unless a source update is independently
   audited and pinned.
2. Accept a full engine only with a clear redistribution grant and provenance
   that does not descend from leaked Source code.
3. Add the engine as a native-source Emscripten build; do not import another
   project's web shell or compiled binary.
4. Expand the exact owner-data manifest to the complete runtime set only after
...
6. Add real HL2 SP/MP variants only when each launches. Add CS:S only when both
   engine and CS:S game-code provenance are legitimate.
