...
4. Delete `hl2/glshaders.cfg` and every `*.dll`. Steam’s leftover GL cache
   crashes the loader; Windows plugins (`trackerui.dll`, `serverbrowser.dll`,
   `hl2/bin/*.dll`) are not used.
...
docker run --rm -p 8088:8088 \
  -v /path/to/your/source-engine:/inputs/source:ro \
