...
framework_image="${SOURCE_WASM_FRAMEWORK_IMAGE:-wasm-game-framework:0.7.1}"
...
git -C "${framework_source_dir}" worktree add --quiet --detach "${framework_dir}" 9359fb186399d0c608cbcd063f7e6fd03eb7c210
cleanup() {
...
for image in "${repository}:${tag}" "${repository}:hl2-${tag}"; do
  [[ "$(docker run --rm --entrypoint node "${image}" -p "require('/opt/wasm-game-framework/package.json').version")" == "0.7.1" ]]
