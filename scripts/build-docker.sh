...
framework_image="${SOURCE_WASM_FRAMEWORK_IMAGE:-wasm-game-framework:0.9.2}"
...
git -C "${framework_source_dir}" worktree add --quiet --detach "${framework_dir}" 53bc7e6eeef1ae35dcf3b25dea4e3ec0ab46726f
cleanup() {
...
for image in "${repository}:${tag}" "${repository}:hl2-${tag}"; do
  [[ "$(docker run --rm --entrypoint node "${image}" -p "require('/opt/wasm-game-framework/package.json').version")" == "0.9.2" ]]
